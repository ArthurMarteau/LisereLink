...
- **Refactorisation EAN par SKU (article + coloris + taille)** : actuellement un EAN identifie un article + coloris uniquement (standard retail classique). L'objectif à terme est d'adopter le modèle Sézane où chaque SKU possède son propre EAN unique incluant la taille (ex : `3400936123458-M`). Cette refactorisation implique : (1) ajout d'un champ `Sku` sur `Article` dans `Lisere.StockApi.Domain`, (2) mise à jour du seed avec un EAN distinct par taille, (3) mise à jour de `GET /api/articles/{barcode}` pour résoudre le SKU → article + taille, (4) sur le scan EAN en `ScanPage`, pré-sélectionner automatiquement la taille résolue sur `ArticleDetailPage`. Cette évolution permet au vendeur de scanner directement l'étiquette d'un article pour identifier immédiatement article, coloris et taille sans manipulation supplémentaire.
